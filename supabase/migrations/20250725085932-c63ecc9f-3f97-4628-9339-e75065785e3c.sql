-- Fix remaining critical RLS security issues (fixed variable naming)

-- Check and enable RLS on remaining tables
DO $$
DECLARE
    tbl_name TEXT;
BEGIN
    -- Check if service_categories table exists and enable RLS
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
        ...
        -- Service categories should be viewable by everyone
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
        ...
        -- System can award achievements (admin only)
